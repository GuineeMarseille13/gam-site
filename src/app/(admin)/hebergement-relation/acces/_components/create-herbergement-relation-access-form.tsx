...
import type { EligiblePersonForHerbergementRelationAccess } from "../_services/get-eligible-persons-for-herbergement-access"
...
interface CreateHerbergementRelationAccessFormProps {
  persons: EligiblePersonForHerbergementRelationAccess[]
...
/**
 * Associe une personne sans compte à un rôle hébergement et mise en relation et un mot de passe.
 */
export function CreateHerbergementRelationAccessForm({
  persons,
  roles,
  action,
}: CreateHerbergementRelationAccessFormProps) {
    ...
    return (
      <Card className="max-w-2xl overflow-hidden border-dashed border-border/70 bg-muted/10">
        <CardContent className="flex flex-col items-center gap-4 px-6 py-14 text-center">
          <span className="flex size-12 items-center justify-center rounded-2xl bg-sky-100 text-emerald-700 dark:bg-sky-950/50 dark:text-sky-300">
    ...
    return (
      <Card className="max-w-2xl overflow-hidden border-dashed border-border/70 bg-muted/10">
        <CardContent className="px-6 py-14 text-center text-sm text-muted-foreground">
          Aucun rôle de hébergement et mise en relation n&apos;est configuré. Contactez un super administrateur.
        </CardContent>
      </Card>
    )
  }
            ...
            <section className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={personId} onValueChange={setPersonId} required>
                  <SelectTrigger id="person-select" className={herbergementRelationAccessSelectTriggerClassName}>
                    <SelectValue placeholder="Rechercher ou choisir…" />
                  </SelectTrigger>
                  <SelectContent className={cn(herbergementRelationAccessSelectContentClassName, "max-h-72")}>
                    {persons.map((p) => (
                      <SelectItem key={p.id} value={p.id} className={herbergementRelationAccessSelectItemClassName}>
            ...
            <section className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className={`${herbergementRelationAccessInputClassName} ${selectedPerson?.email ? "cursor-default bg-muted/40" : ""}`}
