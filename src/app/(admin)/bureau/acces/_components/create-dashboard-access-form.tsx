...
import type { EligiblePersonForAccess } from "../_services/get-eligible-persons-for-access"
import type { DashboardAccessActionResult } from "../_actions/dashboard-access-actions"
import {
  ACCESS_FORM_ICONS,
  AccessSectionTitle,
  SelectedPersonHero,
  accessInputClassName,
  accessSelectContentClassName,
  accessSelectItemClassName,
  accessSelectTriggerClassName,
} from "./access-form-primitives"

interface CreateDashboardAccessFormProps {
  persons: EligiblePersonForAccess[]
  action: (formData: FormData) => Promise<DashboardAccessActionResult>
}
...
/**
 * Associe une personne sans compte à un rôle dashboard et un mot de passe.
 */
export function CreateDashboardAccessForm({ persons, action }: CreateDashboardAccessFormProps) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()
  const [error, setError] = useState<string | null>(null)
  const [fieldErrors, setFieldErrors] = useState<Record<string, string[]> | undefined>()
  const [personId, setPersonId] = useState("")
  const [role, setRole] = useState<string>("BUREAU")
    ...
    return (
      <Card className="max-w-2xl overflow-hidden border-dashed border-border/70 bg-muted/10">
        <CardContent className="flex flex-col items-center gap-4 px-6 py-14 text-center">
          <span className="flex size-12 items-center justify-center rounded-2xl bg-amber-100 text-amber-700 dark:bg-amber-950/50 dark:text-amber-300">
  ...
  return (
    <Card className="max-w-2xl overflow-hidden border-border/50 bg-card shadow-xl shadow-amber-950/[0.04] ring-1 ring-border/30 dark:shadow-black/30">
          ...
          <div className="space-y-8 px-5 py-8 sm:px-8">
            <section className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={personId} onValueChange={setPersonId} required>
                  <SelectTrigger id="person-select" className={accessSelectTriggerClassName}>
                    <SelectValue placeholder="Rechercher ou choisir…" />
                  </SelectTrigger>
                  <SelectContent className={cn(accessSelectContentClassName, "max-h-72")}>
                    {persons.map((p) => (
                      <SelectItem key={p.id} value={p.id} className={accessSelectItemClassName}>
            ...
            <section className="space-y-4">
              <AccessSectionTitle
                ...
                description="Email utilisé pour se connecter au dashboard."
                icon={ACCESS_FORM_ICONS.connection}
              />
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className={`${accessInputClassName} ${selectedPerson?.email ? "cursor-default bg-muted/40" : ""}`}
              ...
              <div className="space-y-2">
                ...
                <Select value={role} onValueChange={setRole}>
                  <SelectTrigger className={accessSelectTriggerClassName}>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className={accessSelectContentClassName}>
                    {SYSTEM_ROLES_SEED.map((r) => (
                      <SelectItem key={r.code} value={r.code} className={accessSelectItemClassName}>
                        <div className="flex flex-col gap-0.5 text-left">
            ...
            <section className="space-y-4">
              <AccessSectionTitle
                ...
                icon={ACCESS_FORM_ICONS.security}
              />
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      className={`${accessInputClassName} pr-10`}
                      aria-invalid={!!fieldError(fieldErrors, "password")}
                    />
                    <button
                      type="button"
                      onClick={() => setShowPwd((v) => !v)}
                      className="absolute right-3 top-1/2 -translate-y-1/2 rounded-md text-muted-foreground transition-colors hover:bg-amber-100/90 hover:text-amber-800 dark:hover:bg-amber-950/50 dark:hover:text-amber-300"
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              <Button
                ...
                className="min-w-[160px] gap-2 rounded-xl bg-amber-500 text-white shadow-md shadow-amber-500/25 transition-colors hover:bg-amber-600 hover:shadow-lg hover:shadow-amber-500/35"
              >
                {isPending ? (
                  <>
              ...
              <Button
                ...
                disabled={isPending}
                onClick={() => router.push("/bureau/acces")}
              >
                Annuler
              </Button>
            </div>
