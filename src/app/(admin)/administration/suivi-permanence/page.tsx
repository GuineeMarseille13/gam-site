...
export default async function SuiviPermanencePage() {
  const [recentRows, demandTypes] = await Promise.all([
  ...
  return (
    <div className="flex flex-1 flex-col gap-8 p-4 md:p-6 lg:p-8">
      <div className="space-y-2">
        ...
        >
          <Link href="/administration/suivi-permanence/types-de-demande">
            <IconList className="size-4" aria-hidden />
            Gérer les types de demande
          </Link>
        </Button>
      </div>
      ...
      <Card className="min-w-0 border-border/60 border-sky-200/50 bg-sky-50/20 shadow-sm dark:border-sky-900/40 dark:bg-sky-950/15">
        ...
        <CardContent className="px-4 pt-0 sm:px-6">
          <BeneficiarySuiviWizard demandTypes={demandTypes} />
        </CardContent>
      </Card>
