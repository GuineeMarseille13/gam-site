...
import type { DashboardAccessListCounts } from "../_services/get-dashboard-access-list"
import type { AccessListStatusFilter } from "../_schemas/access-list-search-params.schema"

const FILTERS: { id: AccessListStatusFilter; label: string; countKey: keyof DashboardAccessListCounts }[] = [
  { id: "all", label: "Tous", countKey: "all" },
  { id: "active", label: "Actifs", countKey: "active" },
  { id: "banned", label: "Suspendus", countKey: "banned" },
]

interface DashboardAccessListToolbarLinksProps {
...
/**
 * Filtres de statut URL-based pour la liste des accès dashboard.
 */
export function DashboardAccessListToolbarLinks({
  scope,
  pathname,
  status,
  counts,
  searchParams,
}: DashboardAccessListToolbarLinksProps) {
  return (
    <div
    ...
    >
      {FILTERS.map(({ id, label, countKey }) => {
        ...
        const href = buildAdminPageHref(
          pathname,
          1,
          { ...searchParams, statut: id === "all" ? undefined : id },
        )
